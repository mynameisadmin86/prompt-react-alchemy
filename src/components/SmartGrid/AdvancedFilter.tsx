...
export function AdvancedFilter({
  ...
  // Convert extra filters to column-like format for rendering
  const extraFilterColumns: GridColumnConfig[] = extraFilters.map(filter => ({
    key: `extra-${filter.field}`,
    label: filter.label,
    type: filter.type === 'select' ? 'Dropdown' : 'Text',
    filterable: true,
    options: filter.options
  }));

  const allFilterableColumns = [...filterableColumns, ...extraFilterColumns];
  ...
  return (
    <div className="space-y-2">
      ...
      {/* Advanced Filter Panel - Only show when showAdvancedFilter is true */}
      {showAdvancedFilter && (
        <div className="bg-white border rounded shadow-sm">
          <div className="grid gap-2 p-3" style={{ gridTemplateColumns: `repeat(${allFilterableColumns.length}, 1fr)` }}>
