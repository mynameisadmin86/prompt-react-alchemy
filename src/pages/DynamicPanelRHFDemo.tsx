...
const DynamicPanelRHFDemo = () => {
  ...
  const getUserPanelConfig = (userId: string, panelId: string): PanelSettings | null => {
    const stored = localStorage.getItem(`panel-config-rhf-${userId}-${panelId}`);
    return stored ? JSON.parse(stored) : null;
  };

  const saveUserPanelConfig = (userId: string, panelId: string, settings: PanelSettings): void => {
    localStorage.setItem(`panel-config-rhf-${userId}-${panelId}`, JSON.stringify(settings));
    console.log(`Saved RHF config for panel ${panelId}:`, settings);
  };
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900">Dynamic Panel RHF Configuration</h1>
            <p className="text-gray-600 mt-1">
              Configure field visibility, ordering, and labels for each panel using React Hook Form
            </p>
          </div>
        ...
        {(basicDetailsVisible || operationalDetailsVisible || billingDetailsVisible) && (
          <div className="bg-white p-6 rounded-lg shadow-sm">
            <h3 className="text-lg font-semibold mb-4">Current Form Data (React Hook Form)</h3>
